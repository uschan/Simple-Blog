...
import toast from "react-hot-toast"; // 添加toast导入
// 导入AI助手组件
...
export default function EditArticlePage() {
  ...
  const handleApplyPolish = (polishedContent: string) => {
    ...
    if (window.confirm("是否将润色后的内容应用到编辑器？")) {
      ...
      if (editor) {
        ...
        editorContentRef.current = polishedContent;
        setEditorContent(polishedContent);
  ...
  const handleApplySeo = (slug: string, keywords: string[]) => {
    setFormData(prev => ({ 
      ...
      tags: Array.from(new Set([...prev.tags, ...keywords])) // 合并并去重
    }));
